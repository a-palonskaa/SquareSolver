...
void PrintTestsReport(testing_t* test, void *storage) {
    ...
    else if (!(roots->n_roots == data->testing_data.roots_expected.n_roots)){
        Log(ERROR, "Error Test No%d a = %lg, b = %lg, c = %lg \n"
                   "number of root n_roots = %d, expected n_roots = %d \n",
                   test->test_number + 1,
                   coeff->a, coeff->b, coeff->c,
                   roots->n_roots, data->testing_data.roots_expected.n_roots);
    }
    else {
...
void TestExecute(testing_t *test, void* storage) {
    ...
    roots->n_roots = QuadraticEquation(coeff->a, coeff->b, coeff->c, &roots->x1, &roots->x2);

    ASSERT_EQUAL(test, data->testing_data.roots_expected.n_roots, roots->n_roots);
...
bool RunAllTests() {
    ...
    const testing_data_t tests[]={  // a,  b,  c,         x1,  x2,          n_Roots
